...
RUN set -x ;\
    git clone --verbose https://github.com/areaDetector/ffmpegServer.git /opt/epics/ffmpegServer && \
    cd /opt/epics/ffmpegServer && \
    git checkout ${FFMPEG_COMMIT}

COPY ./install.sh  /opt/epics/ffmpegServer/install.sh

RUN cd /opt/epics/ffmpegServer/ && \
    bash ./install.sh && \
